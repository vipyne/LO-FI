# LO-FI — a LOcal, personal FInance Pipecat voice AI bot
...
```bash
git clone git@github.com:vipyne/LO-FI.git && cd LO-FI
uv sync
...
```bash
./start.sh               # WebRTC   → open http://localhost:7860/client
./start.sh -t headphones # local audio REQUIRES HEADPHONES 🎧
```
...
│   ├── print_models.py       # print the exact STT/LLM/TTS models in play (used by start.sh/doctor.sh)
...
The bot is a private *thinking partner*, not a licensed financial advisor. _Always_ consult a human after consulting a bot.
